...
class SofaScoreScraper(BaseScraper):
    """Scraper for SofaScore website"""
    
    def __init__(self):
        super().__init__("SofaScore", "https://www.sofascore.com")
    ...
    def scrape_matches(self, league_id: str, date_from: datetime, date_to: datetime) -> List[Dict]:
        ...
        try:
            ...
            logger.info(f"Scraped {len(matches)} matches from SofaScore")
            return matches
            
        except Exception as e:
            logger.error(f"Error scraping matches from SofaScore: {e}")
            return []
    
    def _parse_match_container(self, container) -> Optional[Dict]:
        """Parse individual match container"""
        try:
            # Extract match ID from data attributes
            match_id = container.get('data-match-id', '')
            
            # Extract teams
            team_elements = container.find_all('div', class_='sc-dcJsrY')
            ...
            # Extract score
            score_elem = container.find('div', class_='sc-gsFSXq')
            home_score = None
            away_score = None
            ...
            # Extract time/status
            time_elem = container.find('div', class_='sc-hLBbgP')
    ...
    def scrape_team_stats(self, team_id: str) -> Dict:
        ...
        try:
            ...
            form_container = soup.find('div', class_='sc-kDvujY')
            if form_container:
                form_matches = form_container.find_all('div', class_='sc-jQrLum')
                form = []
                for match in form_matches:
                    result_class = match.get('class', [''])[-1]
                    if 'win' in result_class:
                        form.append('W')
                    elif 'draw' in result_class:
                        form.append('D')
                    elif 'loss' in result_class:
                        form.append('L')
                stats['form'] = form
            
            # Extract statistics
            stats_container = soup.find('div', class_='sc-eCssSg')
            if stats_container:
                stat_rows = stats_container.find_all('div', class_='sc-gsFSXq')
                for row in stat_rows:
                    label_elem = row.find('div', class_='sc-dcJsrY')
                    value_elem = row.find('div', class_='sc-hLBbgP')
            ...
            logger.error(f"Error scraping team stats from SofaScore: {e}")
            return {}
    
    def scrape_h2h(self, team1_id: str, team2_id: str) -> Dict:
        """Scrape head-to-head statistics"""
        try:
            # SofaScore H2H URL structure
            ...
            for match in matches:
                try:
                    # Extract match data
                    teams_elem = match.find('div', class_='sc-dcJsrY')
                    score_elem = match.find('div', class_='sc-gsFSXq')
                    date_elem = match.find('div', class_='sc-hLBbgP')
            ...
            logger.error(f"Error scraping H2H from SofaScore: {e}")
            return {}
    
    def scrape_odds(self, match_id: str) -> Dict:
        """Scrape odds for a match"""
        try:
            # SofaScore odds URL structure
            ...
            # Find odds tables
            odds_tables = soup.find_all('div', class_='sc-eCssSg')
            
            for table in odds_tables:
                try:
                    # Extract bookmaker name
                    bookmaker_elem = table.find('div', class_='sc-dcJsrY')
                    ...
                    # Extract odds
                    odds_rows = table.find_all('div', class_='sc-gsFSXq')
                    bookmaker_odds = {}
            ...
            logger.error(f"Error scraping odds from SofaScore: {e}")
            return {}
    
    def scrape_live_match(self, match_id: str) -> Dict:
        """Scrape live match data"""
        try:
            # SofaScore live match URL
            ...
            score_elem = soup.find('div', class_='sc-gsFSXq')
            if score_elem:
                score_text = score_elem.get_text(strip=True)
                if ':' in score_text:
                    scores = score_text.split(':')
            ...
            # Extract minute
            minute_elem = soup.find('div', class_='sc-hLBbgP')
            if minute_elem:
            ...
            events_container = soup.find('div', class_='sc-kDvujY')
            if events_container:
                events = events_container.find_all('div', class_='sc-jQrLum')
                for event in events:
                    try:
            ...
            logger.error(f"Error scraping live match from SofaScore: {e}")
            return {}
